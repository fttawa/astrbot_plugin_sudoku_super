...
- `advanced_solver_sdk_dir`：可选外部 WASM 运行时目录；留空使用插件内置最小运行时。
...
高级解题模式使用最小 WASM 运行时；仓库不包含 SDK 调用文档、类型声明或完整 SDK 包。运行环境缺少 Node.js 或运行时不可用时，会自动降级为基础候选数提示。
...
- Sudoku WASM 分析引擎：为高级解题模式与候选数分析提供运行时支持。
